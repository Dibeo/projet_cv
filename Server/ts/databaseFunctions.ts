...
interface Data {
  ...
  personneCompetences: PersonneCompetence[];
}
...
export const fetchAllTablesData = async () => {
  try {
    ...
    const personneCompetenceRepository =
      AppDataSource.getRepository(PersonneCompetence);
...
export const dataToHTML = (data: Data): string => {
  let html = `
